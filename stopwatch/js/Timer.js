export default class Timer {
  constructor(root) {
    ...
    this.el.stopStart.addEventListener("click", () => {
      if (this.interval === null) {
        this.start();
        document.getElementsByClassName("outer-circle")[0].style =
          " background: radial-gradient(43.34% 55.31% at 93.34% 44.69%, #000000 0%, #910a0a 100% )";
      } else {
        document.getElementsByClassName("outer-circle")[0].style =
          " background: radial-gradient(43.34% 55.31% at 93.34% 44.69%, #000000 0%, #09A65A 100% )";
        this.stop();
      }
    });
  ...
  start() {
    if (this.remainingSec === 3600) return;

    this.interval = setInterval(() => {
      this.remainingSec++;
      this.updateInterfaceTime();

      if (this.remainingSec === 3600) {
        this.stop();
      }
    }, 1000);
  ...
  static getHTML() {
    return `
        ...
        <div class="timer__btns"> 
          ...
          <span class="timer__btn-control">reset</span>
          </button>
        </div>
        `;
  }
